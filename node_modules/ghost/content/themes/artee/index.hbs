...
<header id="artee-site-head" class="artee-site-head">
	...
	<div class="artee-blog-description">
		<h2>Some people<strong> look</strong> for a beautiful place,</h2>
		<h2> <em>Others<strong> make</strong> a place beautiful</em> .</h2>
	</div>
</header>
{{! header section end }}
...
<main class="artee-index-content" role="main">
	...
	<div id="artee-grid">
	{{#foreach posts}}
		<article class="artee-index-post {{post_class}}" data-groups='["{{#foreach tags}}{{#if @first}}{{name}}{{else}}","{{name}}{{/if}}{{/foreach}}"]' data-title='{{{title}}}' data-date-created='{{date format="YYYY-MM-DD"}}'>
